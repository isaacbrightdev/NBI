...
const CourseCardDataLoader = ({ size = 6, display_vertical, className }) => {
  return (
    <>
      {Array(size)
        .fill()
        .map((e, i) => (
          <div
            key={i}
            className={`${className} course-card relative flex min-w-0 select-none flex-col break-words rounded-[0.625rem] border border-solid border-grey bg-white bg-clip-border p-5`}
          >
            <div className="row">
              <div
                className={`col-12 ${
              ...
              >
                <div className="course-card-image relative aspect-7/4 w-full xl:aspect-7/5">
                  <div className="absolute left-0 top-0 h-full w-full animate-pulse select-none overflow-hidden rounded-[0.625rem] bg-gradient-to-r from-[#def8fe] to-[#b9bec9] object-cover object-center"></div>
                </div>
              </div>
...
CourseCardDataLoader.propTypes = {
  className: PropTypes.string,
  display_vertical: PropTypes.bool,
  size: PropTypes.number
};

export default CourseCardDataLoader;
